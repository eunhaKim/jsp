...
public class PassCheck extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(idx == 1) {
			...
			int key = 0x1234ABCD;
			int encPwd, decPwd;
			encPwd = Integer.parseInt(pwd) ^ key; // xor연산
			
			System.out.println("인코딩된 비밀번호 : " + "1234ABCD" + encPwd);
			System.out.println("앞에서 인코딩(암호화)된 비밀번호를 DB에 저장처리한다. : ");
			System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
			System.out.println("DB에 저장도니 pwd를 다시 불러와서 디코딩 처리한다.");
			...
			// 예) 문자 A로 전송되면 A의 아스키 코드를 변형처리해서 암호화한다.
			long intPwd;
			String strPwd = "";
			// 입력받은 암호를 한문자씩 꺼내어서 아스크코드로 변형 뒤, 문자로 누적처리해서 만들어 준다.
...
//			strPwd = key + encPwd + "";
			...
			// 문자로 변환된 복호화 코드를 가각 2자리씩 잘라서 문자로 변환 후 누적처리...
			for(int i=0 ; i <strPwd.length() ; i+=2) {
				ch = (char) Integer.parseInt(strPwd.substring(i, i+2));
				result += ch;
			}
			System.out.println();
			System.out.println("최종 변환된 비밀 번호(원본 비번과 비교하세요) : " + result);
